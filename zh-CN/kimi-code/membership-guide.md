...
Kimi Code 是 [Kimi 会员权益](https://www.kimi.com/membership/pricing?from=kfc_docs_overview)中专为开发者提供的智能编程服务，基于 Kimi 最新旗舰模型，通过 CLI（Command-Line Interface，命令行界面）、VS Code（Visual Studio Code 编辑器，微软推出的一款轻量级代码编辑器）扩展插件等产品形态，为开发者提供代码阅读、文件编辑、命令执行等 AI 辅助能力。同时，订阅用户可获取 API Key，将 Kimi Code 的模型能力接入到第三方开发工具与平台。
